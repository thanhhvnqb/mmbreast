...
class PydicomMetadata:
    ...
    def __init__(self, ds):
        if "WindowWidth" not in ds or "WindowCenter" not in ds:
            ...
            wc = ds["WindowCenter"]
            self.window_widths = (
                [float(e) for e in ww] if ww.VM > 1 else [float(ww.value)]
            )

            self.window_centers = (
                [float(e) for e in wc] if wc.VM > 1 else [float(wc.value)]
            )
...
def normalize_dicom_img(
    ...
    if method == "windowing":
        assert (
            save_dtype == "uint8"
        ), "Currently `windowing` normalization only support `uint8` save dtype."
        # apply windowing
        if len(window_centers) > 0:
            window_center = window_centers[window_index]
            window_width = window_widths[window_index]
            windowing_backend = (
                "torch" if isinstance(img, torch.Tensor) or force_use_gpu else "np_v2"
            )
            img = apply_windowing(
                img,
        ...
        # do simple min-max scaling
            ...
            print("No windowing param, perform min-max scaling normalization instead.")
...
class _DaliJpegStreamExternalSource:
    ...
    def __call__(self, batch_info):
        ...
        return (
            j_streams,
            np.array(inverts, dtype=np.bool_),
            windowing_params,
            np.array(voilut_funcs, dtype=np.uint8),
        )
...
#         source=eii,
#         num_outputs=4,
#         dtype=[dali.types.UINT8, dali.types.BOOL, dali.types.FLOAT16, dali.types.UINT8],
#         batch=True,
#         batch_info=True,
#         parallel=True,
#     )
#     ori_img = dali.fn.experimental.decoders.image(
#         jpeg, device="mixed", output_type=dali.types.ANY_DATA, dtype=dali.types.UINT16
#     )
#     return ori_img, invert, windowing_param, voilut_func


# def convert_with_dali_ram(
#     dcm_paths,
#     save_paths,
#     save_backend="cv2",
#     save_dtype="uint8",
...
#     external_source = _DaliJpegStreamExternalSource(dcm_paths, batch_size=batch_size)
...
#             fails = dcm_paths[_batch_idx * batch_size : (_batch_idx + 1) * batch_size]
...
#                     dcm_paths[(_batch_idx + 1) * batch_size :], batch_size=batch_size
#                 )
#                 pipe = _dali_decode_pipeline_ram(
#                     external_source,
#                     py_num_workers=py_num_workers,
...
#             feed_ndarray(img_dali, img, cuda_stream=torch.cuda.current_stream(device=0))
...
#     assert cur_idx == len(save_paths) - 1, f"{cur_idx} != {len(save_paths) - 1}"
...
#         jpegs, device="mixed", output_type=dali.types.ANY_DATA, dtype=dali.types.UINT16
#     )
#     return images


# def convert_with_dali_disk(
#     dcm_paths,
#     save_paths,
#     j2k_temp_dir,
#     save_backend="cv2",
...
#         print("Warning: set chunk divided by batch_size for maximum performance.")
#     num_dcms = len(dcm_paths)
...
#             pixel_data = ds.PixelData
#             offset = pixel_data.find(SUID2HEADER[ds.file_meta.TransferSyntaxUID])
#             temp_jpeg_name = os.path.basename(save_path).replace(".png", ".temp")
...
#             metas.append(meta)
#         # dali to process with chunk in-memory
#         pipe = _dali_decode_pipeline_disk(
#             temp_j2k_paths,
#             batch_size=batch_size,
#             num_threads=num_threads,
#             device_id=device_id,
...
#                 img = torch.empty(img_dali.shape(), dtype=torch.int16, device="cuda")
#                 feed_ndarray(
#                     img_dali, img, cuda_stream=torch.cuda.current_stream(device=0)
#                 )
#                 img = normalize_dicom_img(
#                     img,
#                     invert=meta.invert,
...
#         assert (
#             cache != "disk"
#         ), "Currently, cache method `disk` can not be used in parallel."
...
#         ends = [min(start + num_samples_per_worker, num_samples) for start in starts]
#         if isinstance(device_id, list):
#             assert len(device_id) == parallel_n_jobs
#         elif isinstance(device_id, int):
#             device_id = [device_id] * parallel_n_jobs

#         print(f"Starting {parallel_n_jobs} jobs with backend `{parallel_backend}`...")
...
#################################### DICOMSDL ####################################
def _convert_single_with_dicomsdl(
    dcm_path,
    save_path,
    normalization="windowing",
    save_backend="cv2",
...
def convert_with_dicomsdl_parallel(
    dcm_paths,
    save_paths,
    normalization="windowing",
    save_backend="cv2",
    ...
    if parallel_n_jobs == 1:
        ...
        print(f"Starting {parallel_n_jobs} jobs with backend `{joblib_backend}`")
        _ = Parallel(n_jobs=parallel_n_jobs, backend=joblib_backend)(
            delayed(_convert_single_with_dicomsdl)(
                dcm_paths[j],
                save_paths[j],
                normalization=normalization,
                save_backend=save_backend,
...
def _convert_single_with_pydicom(
    dcm_path,
    save_path,
    normalization="windowing",
    save_backend="cv2",
...
def convert_with_pydicom(
    ...
    for dcm_path, save_path in tqdm(zip(dcm_paths, save_paths)):
        _convert_single_with_pydicom(
            dcm_path, save_path, normalization, save_backend, save_dtype
        )


def convert_with_pydicom_parallel(
    dcm_paths,
    save_paths,
    normalization="windowing",
    save_backend="cv2",
    ...
    if parallel_n_jobs == 1:
        print("No parralel. Starting the tasks within current process.")
        return convert_with_pydicom(
            dcm_paths, save_paths, normalization, save_backend, save_dtype
        )
    else:
        print(f"Starting {parallel_n_jobs} jobs with backend `{parallel_backend}`...")
        _ = Parallel(n_jobs=parallel_n_jobs, backend=parallel_backend)(
            delayed(_convert_single_with_pydicom)(
                dcm_paths[j], save_paths[j], normalization, save_backend, save_dtype
            )
            for j in tqdm(range(len(dcm_paths)))
        )
...
####################################### NVJPEG2K #######################################
def _convert_single_with_nvjpeg2k(
    j2k_decoder,
    dcm_path,
    save_path,
    normalization="windowing",
...
def convert_with_nvjpeg2k_parallel(
    dcm_paths,
    save_paths,
    normalization="windowing",
    save_backend="cv2",
...
# def _convert_single_with_dali_eager(
#     dcm_path,
#     save_path,
#     normalization="windowing",
#     save_backend="cv2",
...
#     output = eager.experimental.decoders.image(
#         [j2k_stream],
#         device="gpu",
...
#     assert len(dcm_paths) == len(save_paths)
#     for i in tqdm(range(len(dcm_paths))):
#         _convert_single_with_dali_eager(
#             dcm_paths[i], save_paths[i], normalization, save_backend, save_dtype
#         )


# def convert_with_dali_eager_parallel(
#     dcm_paths,
#     save_paths,
#     normalization="windowing",
#     save_backend="cv2",
...
#     if parallel_n_jobs == 1:
#         print("No parralel. Starting the tasks within current process.")
#         return convert_with_dali_eager(
#             dcm_paths, save_paths, normalization, save_backend, save_dtype
#         )
#     else:
#         print(f"Starting {parallel_n_jobs} jobs with backend `{parallel_backend}`...")
