...
__all__ = [
    "stage1_process_rsna",
    "stage1_process_vindr",
    "stage1_process_miniddsm",
    "stage1_process_cmmd",
    "stage1_process_cddcesm",
    "stage1_process_bmcd",
]
...
def get_dicom_meta(dicom_root_dir, dicom_df, extension="dcm", verbose=False):
    dicom_data = dict()
    keywords = set()
    dicom_df = dicom_df[["patient_id", "image_id"]].copy().reset_index(drop=True)

    for i in tqdm(range(len(dicom_df))):
        row = dicom_df.loc[i]
        patient_id = row["patient_id"]
        image_id = row["image_id"]
        dicom = get_dicom_meta_from_file(
            dicom_root_dir, patient_id, image_id, extension
        )
    ...
    for keyword in keywords:
        dicom_df[keyword] = dicom_df[["patient_id", "image_id"]].apply(
            lambda x: (
                np.nan
                if keyword not in dicom_data[x.patient_id][x.image_id]
                else dicom_data[x.patient_id][x.image_id][keyword]
            ),
            axis=1,
        )
    if verbose:
...
def stage1_process_rsna(
    ...
    dicom_df.rename(
        columns={name: "__" + name for name in dicom_df.columns if name != "image_id"},
        inplace=True,
    )
    dicom_df["__WindowCenterList"] = dicom_df["__WindowCenter"].apply(
        lambda x: [float(x)] if isinstance(x, float) else [float(e) for e in x]
    )
    dicom_df["__WindowCenterListLength"] = dicom_df["__WindowCenterList"].apply(len)
    dicom_df["__WindowWidthList"] = dicom_df["__WindowWidth"].apply(
        lambda x: [float(x)] if isinstance(x, float) else [float(e) for e in x]
    )
    dicom_df["__WindowWidthListLength"] = dicom_df["__WindowWidthList"].apply(len)
    merged = pd.merge(
        df,
        dicom_df,
        how="inner",
...
########################### VINDR ###########################
def stage1_process_vindr(
    raw_root_dir, stage1_images_dir, cleaned_label_path, force_copy=False
):
    del force_copy
    dcm_root_dir = os.path.join(raw_root_dir, "images")
    df = pd.read_csv(os.path.join(raw_root_dir, "breast-level_annotations.csv"))
    df.rename(
        columns={
            "study_id": "patient_id",
            "view_position": "view",
    ...
    merged = merged[
        [
            ...
            "__StudyInstanceUID",
            "__ViewPosition",
            "__LargestImagePixelValue",
            "__PixelSpacing",
            "__PixelPaddingRangeLimit",
            "__SeriesInstanceUID",
            "__SamplesPerPixel",
            "__PresentationLUTShape",
            "__WindowCenterWidthExplanation",
            "__WindowCenter",
            "__ImagerPixelSpacing",
            "__VOILUTFunction",
            "__RescaleIntercept",
            "__RescaleSlope",
...
########################### MINI-DDSM ###########################
def stage1_process_miniddsm(
    raw_root_dir, stage1_images_dir, cleaned_label_path, force_copy=False
):
    df = pd.read_excel(
        os.path.join(
            raw_root_dir, "Data-MoreThanTwoMasks", "Data-MoreThanTwoMasks.xlsx"
        )
    )
...
def stage1_process_cmmd(
    ...
    dicom_df = pd.DataFrame({"patient_id": all_patient_ids, "image_id": all_image_ids})
    ...
    merged["cancer"] = merged.classification.apply(
    ...
    merged = merged[
        [
            ...
            "__CodeMeaning",
            "__StudyID",
            "__PatientOrientation",
            "__SpecificCharacterSet",
            "__InstanceCreatorUID",
            "__PositionerType",
            "__PhotometricInterpretation",
            "__BitsAllocated",
            "__StudyDate",
            ...
            "__Modality",
            "__PresentationIntentType",
            "__AcquisitionTime",
            "__BodyPartExamined",
            "__HighBit",
            "__PatientIdentityRemoved",
            "__BitsStored",
            "__InstanceCreationTime",
            "__StudyTime",
            "__SeriesTime",
            "__PatientBirthDate",
            "__ImageType",
            ...
            "__AcquisitionDate",
            "__PatientAge",
            "__BurnedInAnnotation",
            "__PresentationLUTShape",
            "__Manufacturer",
            "__DeidentificationMethod",
            "__ImagerPixelSpacing",
            "__PatientSex",
        ]
    ]
...
########################### CDD-CESM ###########################
def stage1_process_cddcesm(
    raw_root_dir, stage1_images_dir, cleaned_label_path, force_copy=False
):
    df = pd.read_excel(
        os.path.join(raw_root_dir, "Radiology-manual-annotations.xlsx"),
        sheet_name="all",
    )
    ...
    for i in tqdm(range(len(df))):
        ...
        image_id = df.at[i, "image_id"]
        image_name = image_id.replace("L_", "L_DM_").replace("R_", "R_DM_") + ".jpg"
        patient_dir = os.path.join(stage1_images_dir, patient_id)
        os.makedirs(patient_dir, exist_ok=True)
        src_path = os.path.join(
            raw_root_dir, "CDD-CESM", "Low energy images of CDD-CESM", image_name
        )
...
def stage1_process_bmcd(
    ...
    df = df[df.biopsy_result.isin(["NAN", "BENIGN", "DCIS", "MALIGNANT"])].reset_index(
        drop=True
    )
    ...
    for i in tqdm(range(len(df))):
        ...
        for ori_name in names:
            ...
            new_s = s.copy()
            if (
                new_s["biopsy_result"] == "DCIS"
                or new_s["biopsy_result"] == "MALIGNANT"
            ):
                if "prior" in name:
    ...
    df["cancer"] = df.biopsy_result.apply(
        lambda x: 1 if x in ["MALIGNANT", "DCIS"] else 0
    )
    df.rename(
        columns={
            "dir": "patient_id",
